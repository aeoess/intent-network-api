...
import { cardContentHash, validateV3Card } from './v3-cards.js';
...
function rateLimited(action, keyOf) {
    return (req, res, next) => {
...
function verifyCardCrypto(card) {
...
router.post('/cards', rateLimited('publish', req => String(req.body?.card?.subject_key ?? '')), async (req, res) => {
    ...
    if (!crypto.ok) {
        res.status(403).json({ error: crypto.error });
        return;
    }
    ...
    if (card.revocation_status !== 'active') {
        res.status(400).json({ error: 'a new card must be published with revocation_status active' });
        return;
    }
    ...
    try {
        ...
        if (text.length > 0) {
            ...
            if (vec)
                v3db.storeV3Embedding(cardId, vec);
        }
    }
    catch (e) {
        console.error('[v3] embedding failed (card still published):', e.message);
    }
...
router.post('/cards/search', rateLimited('search', req => String(req.headers['x-public-key'] ?? '')), async (req, res) => {
    ...
    if (typeof query === 'string' && query.trim().length > 0) {
        try {
            ...
            if (!vec) {
                res.status(500).json({ error: 'semantic search unavailable: embedding model not ready' });
                return;
            }
            ...
            res.status(500).json({ error: 'semantic search unavailable: ' + e.message });
            return;
        }
    }
...
function requireVerbSignature(req, res, cardId, verb) {
    const { signature, public_key } = req.body ?? {};
    if (!signature || !public_key) {
        res.status(401).json({ error: 'signature and public_key required' });
        return null;
    }
    ...
    if (stored.card.subject_key !== public_key) {
        res.status(403).json({ error: 'not the card subject' });
        return null;
    }
    try {
        if (!verify(`${verb}:${cardId}`, signature, public_key)) {
            res.status(403).json({ error: 'verb signature does not verify' });
            return null;
        }
    }
    catch (e) {
        res.status(403).json({ error: `verb signature verification failed: ${e.message}` });
        return null;
    }
    return public_key;
}
...
//# sourceMappingURL=v3-routes.js.map