...
export function insertV3Card(cardId, card, cardHash) {
    d().prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(cardId, card.card_type, card.subject_key, cardHash, JSON.stringify(card), card.headline, JSON.stringify(card.intents), card.event_ref?.event_id ?? null, card.created_at, card.expires_at, card.revocation_status);
}
export function getV3Card(cardId) {
    const row = d().prepare('SELECT card_id, card_json, revocation_status, expires_at FROM v3_cards WHERE card_id = ?').get(cardId);
    if (!row)
        return null;
...
export function setRevocationStatus(cardId, subjectKey, status) {
    const res = d().prepare(`
...
export function deleteV3Card(cardId, subjectKey) {
...
export function storeV3Embedding(cardId, vector) {
...
export function removeFromIndex(cardId) {
    const rows = d().prepare('SELECT rowid_ref FROM v3_embedding_cards WHERE card_id = ?').all(cardId);
    for (const r of rows)
        d().prepare('DELETE FROM v3_embeddings WHERE rowid = ?').run(r.rowid_ref);
    d().prepare('DELETE FROM v3_embedding_cards WHERE card_id = ?').run(cardId);
}
export function semanticSearchV3(queryVec, limit) {
    ...
    ORDER BY e.distance
  `).all(Buffer.from(queryVec.buffer, queryVec.byteOffset, queryVec.byteLength), limit * 3);
    return rows.map(r => ({ card_id: r.card_id, distance: r.distance }));
}
const SEARCH_CAP = 50;
export function searchV3Cards(filters, semanticIds, limit = 20) {
    ...
    if (filters.event_ref) {
        where.push('event_ref_id = ?');
        params.push(filters.event_ref);
    }
    const rows = d().prepare(`SELECT card_id, card_json, revocation_status FROM v3_cards WHERE ${where.join(' AND ')} ORDER BY created_at DESC LIMIT 500`).all(...params);
    const results = [];
    for (const row of rows) {
        const card = JSON.parse(row.card_json);
        card.revocation_status = row.revocation_status;
        if (semanticIds && !semanticIds.includes(row.card_id))
            continue;
        if (filters.intents?.length && !filters.intents.some(i => card.intents.includes(i)))
            continue;
        if (filters.topics?.length) {
            const cardTopics = [...card.seeking, ...card.offering].flatMap(e => e.topics ?? []);
            if (!filters.topics.some(t => cardTopics.some(ct => ct.toLowerCase().includes(t.toLowerCase()))))
                continue;
        }
        if (filters.engagement) {
            const engagements = card.seeking.map(s => s.engagement).filter(Boolean);
            const prefEngagement = card.preferences.filter(p => p.key === 'engagement').map(p => p.value);
            if (![...engagements, ...prefEngagement].some(e => e.toLowerCase().includes(filters.engagement.toLowerCase())))
                continue;
        }
        if (filters.location) {
            const locPrefs = card.preferences.filter(p => p.key === 'location').map(p => p.value);
            if (!locPrefs.some(l => l.toLowerCase().includes(filters.location.toLowerCase())))
                continue;
        }
...
export function sweepExpiredV3Cards() {
    const expired = d().prepare(`SELECT card_id FROM v3_cards WHERE expires_at <= ${SQL_NOW_ISO} AND revocation_status != 'deleted'`).all();
    for (const row of expired)
        removeFromIndex(row.card_id);
    const res = d().prepare(`
...
export function v3CardCount() {
    return d().prepare(`SELECT COUNT(*) AS n FROM v3_cards WHERE expires_at > ${SQL_NOW_ISO} AND revocation_status = 'active'`).get().n;
}
//# sourceMappingURL=v3-db.js.map