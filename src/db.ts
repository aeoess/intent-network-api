...
export function ensureProfile(publicKey: string, agentId: string): void {
  const d = getDb()
...
export function incrementProfile(publicKey: string, field: string): void {
  const d = getDb()
...
export function getTrustSignals(agentId: string): any {
  const d = getDb()
  const profile = d.prepare('SELECT * FROM identity_profiles WHERE agent_id = ?').get(agentId) as any
  if (!profile) return { identityAge: 0, responseRate: 0, acceptanceRate: 0, trustLevel: 'new' }
  ...
  let trustLevel = 'new'
  if (ageDays >= 1 && profile.total_intros_accepted >= 1) trustLevel = 'established'
  if (ageDays >= 7 && profile.total_intros_accepted >= 3 && responseRate >= 50) trustLevel = 'trusted'
  if (ageDays >= 30 && profile.total_intros_accepted >= 10 && responseRate >= 70) trustLevel = 'veteran'
...
export function submitFeedback(introId: string, fromAgent: string, rating: 'useful' | 'neutral' | 'not_useful', comment?: string): boolean {
  const d = getDb()
  try {
    ...
    const intro = d.prepare('SELECT requested_by, target_agent_id FROM intros WHERE intro_id = ?').get(introId) as any
    if (intro) {
      const otherAgent = fromAgent === intro.requested_by ? intro.target_agent_id : intro.requested_by
      const otherCard = d.prepare('SELECT public_key FROM cards WHERE agent_id = ?').get(otherAgent) as any
      if (otherCard) {
        if (rating === 'useful') incrementProfile(otherCard.public_key, 'total_feedback_useful')
        else if (rating === 'not_useful') incrementProfile(otherCard.public_key, 'total_feedback_not_useful')
        else incrementProfile(otherCard.public_key, 'total_feedback_neutral')
      }
    }
    return true
  } catch { return false }
...
export function updateLinkedProofs(publicKey: string, githubUrl?: string, websiteUrl?: string): void {
  const d = getDb()
  const updates: string[] = []
  const params: any[] = []
  if (githubUrl !== undefined) { updates.push('github_url = ?'); params.push(githubUrl || null) }
  if (websiteUrl !== undefined) { updates.push('website_url = ?'); params.push(websiteUrl || null) }
  if (updates.length > 0) {
